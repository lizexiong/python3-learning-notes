...
def auth(client_socket,args):
    ...
    #将传入的用户对象实例化
    #首先通过实例化一个Users对象,判断用户是否在里面
    client_user = Users(username)

    #User类如果存在这个用户，那么字段exists就为True
...
def put(client_socket, client_user, recv_data):
    ...
    try:
        ...
        with open(save_path,'a+b') as fa:
            ...
            #剩下的不足2048了
                ...
                if recv_data == b'':
                    #写入文件,为以后断点续传准备
                    dbapi.write_breakpoint(filemd5,filesize,recv_size,save_path,client_user)
                    common.write_log("Client upload file connected closed", "error")
                fa.write(recv_data)
        ...
        if recv_data < filesize:
            #如果出现异常，也记录一个端点信息
            dbapi.write_breakpoint(filemd5, filesize, recv_size, save_path, client_user)
        common.write_log(str(e), "error")
...
def show(client_socket, client_user, recv_data):
    ...
    if file_count > 0:
        ...
        #循环每个文件的信息经过处理，然后累加到return_list里面发送给客户端
        for i in file_list:
    ...
    try:
        ...
        #这里就没有做每次2048发送,当然也是可以那么做的，这里为了方便，一下子全部发送过去了
        if read_stat == "ready":
