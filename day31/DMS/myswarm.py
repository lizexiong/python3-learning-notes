...
class Myswarm(object):
    ...
    def _container_detail(self, node_ip, node_port, containers_id):
        ...
        ret_json = json.loads(container_more_url.get().decode())
        return ret_json
    ...
    def container_list(self, node_ip, node_port):
        ...
        # print (help(container_url))

        #这里和7年前不太一样，没有get_value方法了
        ret_json = json.loads(container_url.get().decode())

        # print (type(ret_json),ret_json)
        con_data = {}
        container_id = []
        if ret_json:
            for i in ret_json:
                pass
                container_id.append(i['Id'])
    ...
    def images_list(self, node_ip, node_port):
        client_ins = docker.APIClient(base_url='tcp://' + node_ip + ':'
                                            + node_port, version='1.21', timeout=5)
    ...
    def create_container(self, node_ip, node_port, conf):
        client_ins = docker.APIClient(base_url='tcp://' + node_ip + ":" + node_port, version='1.39', timeout=5)
    ...
    def start_container(self, node_ip, node_port, container_id):
        if len(container_id) > 0:
            container_ip = ""
            client_ins = docker.APIClient(base_url='tcp://' + node_ip + ":" + node_port, version='1.21', timeout=5)
